...
export function Header() {
  return (
    <header className="sticky top-0 z-50 border-b border-white/50 bg-[#eef3f8]/78 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group inline-flex min-w-0 items-center gap-3">
          <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-base-ink text-sm font-semibold text-white shadow-glass transition group-hover:scale-105">
            GY
          </span>
          <span className="min-w-0">
            <span className="block truncate text-sm font-semibold text-base-ink">{site.shortName}</span>
            <span className="block text-xs text-base-soft">Digital Base</span>
          </span>
        </Link>

        <nav className="hidden items-center gap-1 rounded-full border border-white/60 bg-white/50 p-1 shadow-inset backdrop-blur md:flex">
          {navigation.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-base-soft transition hover:bg-white/85 hover:text-base-ink"
            >
              {item.title}
            </Link>
          ))}
        ...
        <Link
          ...
          className="inline-flex shrink-0 items-center gap-2 rounded-full bg-base-ink px-4 py-2 text-sm font-medium text-white shadow-glass transition hover:-translate-y-0.5 hover:bg-slate-700"
        >
